...
import { INDEX_MAX_BLOCKS, SPAWN_BLOCK, THEBALL_ACCOUNT } from './config.js';
...
export async function tickIndexer() {
  ...
  if (toBlock <= lastBlock) {
    if (!lastBlockStr) await setMeta('last_block', headBlock.toString());
    return { processed: 0, headBlock };
  }
      ...
      if (opName === 'custom_json') {
        ...
        if (id === 'theball') {
          ...
          await sql`
            ...
            VALUES (${json.ball || null}, ${(json.op || json.type) || 'unknown'}, ${signer}, ${opData.json}, ${blockNum}, ${tx.trx_id}, ${tx.timestamp}, ${result.valid}, ${result.reason || null})
          ...
          if (result.valid) {
            ...
            if ((json.op || json.type) === 'register' || (json.op || json.type) === 'drop') {
              ...
              `;
            } else if (['spawn', 'throw', 'catch', 'bounce', 'dead'].includes((json.op || json.type))) {
          ...
          if (Array.isArray(json) && json[0] === 'follow') {
            ...
            if (followData.following === THEBALL_ACCOUNT) {
